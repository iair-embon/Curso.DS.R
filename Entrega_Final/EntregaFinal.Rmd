...
```{r}
spam_train <- d %>% select(Anhedonia:Withdrawal) %>% data.matrix()
spam_y_train <- d %>% pull(overconfidence)



map(
  1:nrow(spam_train),
  ~ glmnet(spam_train[-.x, ], spam_y_train[-.x], family = "binomial",
                            alpha = 0, lambda = seq(0.01, 20,length.out = 100)) %>% 
    predict(spam_train[.x, ], type = 'response')
) %>% map(
  ~ ifelse(.x > 0.5, 1, 0)
) %>% unlist() %>% matrix(nrow = nrow(spam_train), ncol = 100, byrow = T)
```
