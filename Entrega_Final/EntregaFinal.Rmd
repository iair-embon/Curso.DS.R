...
Crear un vector llamado `columnas_dominio` que contenga los nombres de las columnas correspondientes a los 5 dominios a evaluar. 

```{r}
columnas_dominio <- colnames(d)[str_detect(colnames(d), pattern = 'Domain')]
```

Crear un vector que se llame `mejor_subconjunto` que contenga strings con todas todas las posibles combinaciones de predictores desde un solo predictor hasta el total de predictores. La estructura de cada string debe ser "Var1 + Var2 + Var3 ..." donde "Var1" indica el nombre del primero de los predictores utilizados en ese caso y asi sucesivamente. 

```{r}
mejor_subconjunto <- sapply(
  1:5, function(x) combn(columnas_dominio, x, simplify = F) %>% 
                 lapply(function(y) str_flatten(y, ' + ')) %>% 
                 unlist()
       )  %>% unlist()
```

Armar dos vectores llamados `formula_dominio_simple` y `formula_dominio_completa` que tengan una string con todos los posibles predictores unicos y todos los predictores juntos, respectivamente. En ambos casos se debe concatenar al principio de la string el termino 'overconfidence ~'

```{r}
formula_dominio_simple <- paste0(
  'overconfidence ~ ', mejor_subconjunto[1:5]
)

formula_dominio_completa <- as.formula(
  paste0(
    'overconfidence ~ ', mejor_subconjunto[length(mejor_subconjunto)]
         )
  )
```
...
Evaluar un ajuste polinomial para la regresion logistica. Para esto generar una grilla de posibles valores para el grado del polinomio que vaya de 1 a 30 (con intervalos de 1) y ajustar el modelo usando la funcion `poly`. Para cada grado posible del polinomio utilizar un procedimiento leave-one-out para calcular el accuracy y elegir asi el grado optimo del polinomio a utilizar.

Calcular el intervalo de confianza del 95% del predictor con el grado optimo del polinomio escogido en el punto anterior. 